...
import { rejectionEmail } from 'src/emails-templates/rejectionEmail';
import { link } from 'fs';
@Injectable()
export class MailService {
  ...
  //             .header { text-align: center; background-color: #4caf50; color: white; padding: 10px 0; font-size: 20px; font-weight: bold; border-radius: 8px 8px 0 0; }
  //             .content { font-size: 16px; line-height: 1.5; color: #333333; margin: 20px 0; }
  //             .footer { margin-top: 20px; text-align: center; font-size: 12px; color: #777777; }
  //           </style>
  //         </head>
  //         <body>
  //           <div class="container">
  //             <div class="header">Interview Platform Notification</div>
  ...
  async sendInterviewReminderEmail(to: string, interviewTime: string, interviewDate: string, recipientName: string, interviewLink: string) {
    ...
    const htmlContent = generateInterviewReminder(name, interviewDate, interviewTime, interviewLink);
    try {
      await this.mailerService.sendMail({
        to,
        subject,
